...
- deploy (manual) → workflow_dispatch with blue-green deployment to Cloud Run
  ...
  - Step 2: Validate `/health` endpoint (5 retries, 10s intervals)
...
1. **Deploy (no traffic):** New revision deployed with `--no-traffic` flag, tagged as 'blue'
2. **Validate:** Health check on new revision URL (5 attempts, 10s intervals, must return HTTP 200)
