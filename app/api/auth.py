...
async def connect_to_adobe(request: Request, oauth_service: OAuthService = Depends(get_oauth_service_dependency)):
    redirect_uri = request.url_for('auth_adobe')
    return await oauth_service.login(request, redirect_uri)

@router.get("/auth/adobe", name="auth_adobe")
async def auth_adobe(request: Request, oauth_service: OAuthService = Depends(get_oauth_service_dependency)):
